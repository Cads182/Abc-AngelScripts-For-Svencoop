...
class weapon_dmbow : ScriptBasePlayerWeaponEntity
{
	...
	void PrimaryAttack()
	{
		...
		if( m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD )
			vecSpeed = vecDir * 1200;
		else
			vecSpeed = vecDir * 1700;
		
		if(!bInZoom)
		{
			CBaseEntity@ pBolt = g_EntityFuncs.Create("crossbow_bolt", vecSrc + g_Engine.v_forward * 15, m_pPlayer.pev.v_angle, false, m_pPlayer.edict());
		...
		{
			g_DMUtility.DMTraceDamage( m_pPlayer, self.pev,1, vecSrc, vecAiming, g_vecZero, 8192, BULLET_PLAYER_CUSTOMDAMAGE, m_iBulletDamage ,false);
			TraceResult tr;	
			float x, y;	
...
void RegisterDMXBOW()
{
	...
	g_DMEntityList.insertLast(GetWeaponNameDMXBOW());
}